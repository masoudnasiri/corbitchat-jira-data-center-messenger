...
import com.corbitlogic.jira.internalmessenger.dto.DevTestAssignmentRequestDto;
...
@Path(value="/dev/test-assignment-card")
@Consumes(value={"application/json"})
@Produces(value={"application/json"})
public class JimDevTestAssignmentResource {
    private static final Logger log = LoggerFactory.getLogger(JimDevTestAssignmentResource.class);
    ...
    @Inject
    public JimDevTestAssignmentResource(JiraAuthenticationContext authenticationContext, GlobalPermissionManager globalPermissionManager, JimMessageService messageService, JimRestJsonMapper restJsonMapper) {
    ...
    @POST
    public Response createTestAssignmentCard(DevTestAssignmentRequestDto request) {
        ...
        try {
            ...
            String body = actorDisplayName + " assigned " + issueKey + " to you.";
            String fingerprint = JimConversationKeys.buildEventFingerprint(JimEventType.ASSIGNMENT.name(), targetUserKey, issueKey, "dev-test", "dev-" + System.currentTimeMillis());
            JimMessage message = this.messageService.createSystemMessage(targetUserKey, JimEventType.ASSIGNMENT, null, actorDisplayName, issueKey, issueSummary, issueUrl, body, fingerprint);
            if (message == null) {
                return JimRestResponses.errorJson(409, "duplicate_event", "A test assignment card with this fingerprint already exists.");
            }
            ...
            return JimRestResponses.internalError(log, "POST /rest/jim/1.0/dev/test-assignment-card", user.getKey(), ex, "internal_error", "An internal error occurred while creating the test assignment card.");
        }
    }
}
